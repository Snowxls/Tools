...
- (id)tn_objectForKey:(NSString *)key {
    if (![self.class isKindOfClass:[NSDictionary class]]) {
        return nil;
    }
    return [self tn_objectForKey:key];
}
...
@implementation WJSafeCollection
...
//        [self exchangeOriginalMethod:class_getClassMethod([NSDictionary class], @selector(objectForKey:)) withNewMethod:class_getClassMethod([NSDictionary class], @selector(tn_objectForKey:))];
        // NSMutableDictionary
